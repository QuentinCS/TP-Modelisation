...
mu_rho_tot = np.zeros(4)

for energy in range(0, 4):
    mu_rho_tot[energy] = mu_rho_Rayleigh[energy] + mu_rho_compton[energy] + mu_rho_pair[energy] + mu_rho_photo_elec[energy]
...
for energy in range(4):
    for i in range(Nb):  
        ...
        if (R_2 < mu_rho_Rayleigh[energy]/mu_rho_tot[energy]):
            Nb_Ray[energy] += 1
        if (R_2 > mu_rho_Rayleigh[energy]/mu_rho_tot[energy] and R_2 < (mu_rho_Rayleigh[energy]/mu_rho_tot[energy]+mu_rho_photo_elec[energy])/mu_rho_tot[energy]):
            Nb_photo[energy] += 1
        if (R_2 > mu_rho_photo_elec[energy]/mu_rho_tot[energy] and R_2 < (mu_rho_compton[energy]+mu_rho_photo_elec[energy])/mu_rho_tot[energy]):
            Nb_compton[energy] += 1
        if (R_2 > (mu_rho_compton[energy]+mu_rho_photo_elec[energy])/mu_rho_tot[energy] and E_int[energy] > 1.022):
            Nb_paire[energy] += 1
